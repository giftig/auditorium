...
trait ElasticsearchReporting extends JsonSupport {
  ...
  private def recordResponse[T](r: (Try[HttpResponse], T)): (Try[HttpResponse], T) = {
    r._1 match {
      ...
      case Success(resp: HttpResponse) if resp.status.isFailure => logger.error(
        ...
        s"response content: ${resp.entity}"
      )
    }
    r
  }
  ...
  /**
   * Get the name of the index to use, allowing offsetting the index to get the next index, etc.
   *
   * @param offset The index number to offset by, i.e. 0 for the current one, 1 for the next...
   */
  def indexName(offset: Int = 0): String = {
    val now = new GregorianCalendar()
    now.add(Calendar.MONTH, offset)
    val year = now.get(Calendar.YEAR)
    val month = now.get(Calendar.MONTH) + 1
    f"$indexPrefix%s$year%04d$month%02d"
  }

  private lazy val attachIndex: Flow[Report, ReportWithIndex, NotUsed] = Flow.fromFunction {
    ReportWithIndex(_, indexName())
  }
